...
def cat_copy_func_kernel_4(
    ...
    # Source index decomposition: keep as i32 so mode1 uses f32 NPU vectorization.
    # The divisors here are always powers of 2 or small enough that the f32 reciprocal
    # approximation is exact for the index ranges encountered in practice.
    pre_idx = idx // (dim_size_in * dim_prod_post)
    dim_idx = (idx // dim_prod_post) % dim_size_in
    post_idx = idx % dim_prod_post

    # Destination index: cast to i64 before the large multiply so that mode1 keeps
    # this computation as exact integer arithmetic (i64 is preserved in mode1 while
    # i32 is vectorized via f32). Without i64, out_idx > 2^24 loses precision in f32.
    out_idx = (
        pre_idx.to(tl.int64) * dim_size_out * dim_prod_post
        + (dim_idx + dim_offset).to(tl.int64) * dim_prod_post
        + post_idx.to(tl.int64)
    )
