...
def argmin_kernel_opt_k1(
    ...
    # Use input dtype's max as fill value; always accumulate in float32
    inp_max = get_dtype_max(inp.type.element_ty)
    fp32_max = get_dtype_max(tl.float32)
    min_vals = tl.full([BLOCK_M], dtype=tl.float32, value=fp32_max)
    ...
    for start_n in range(0, N, BLOCK_N):
        ...
        inp_vals = tl.load(
            inp + offset, mask=m_mask[:, None] & n_mask[None, :], other=inp_max
        ).to(tl.float32)

        # Split into separate tl.min + tl.argmin to avoid broken return_indices on backend
        local_min = tl.min(inp_vals, 1)
    ...
    tl.store(out_index + m_offset, argmin_vals, mask=m_mask)
